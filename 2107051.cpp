...
private:
    ...
    int totalMonths;
public:
    Account(string n,int number,string type,double initialBalance):name(n),
...
public:
    void addAccount( string name, int number,  string type, double initialBalance);
...
void Bank::addAccount( string name, int number,  string type, double initialBalance) {
...
void Bank::displayAccountDetails(int accountNumber){
    ...
    if (account){
        ...
        if (account->getIsLoanTaker()) {
            cout << "Remaining Loan: " << account->getRemainingLoan() << endl;
